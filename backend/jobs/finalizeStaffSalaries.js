...
// The 1st finalizes the PREVIOUS calendar-month period — same convention
...
async function runFinalizeStaffSalariesJob() {
    ...
    for (const { person_type, PersonModel } of STAFF_TYPES) {
        ...
        // Rows are already created as "non payé" by the generation job, so this
        // is now a safety net for any leftover legacy "en attente" rows rather
        // than the primary status transition — never overwrites "payé".
    ...
    if (isWithinPayrollWindow(now)) {
        ...
        console.log(`[${JOB_NAME}] teachers: recalculated ${teacherResults.length} as non payé for ${period}`);
    } else {
